...
export function EnchantCard({ e }: { e: Encantamento }) {
  const color = rarityColor[e.raridade] ?? "text-text-muted border-border-bright";
  return (
    <div className="flex h-full flex-col gap-1 rounded-xl border border-border bg-bg-card p-4 transition-colors hover:border-accent/40">
      <div className="flex items-start justify-between gap-2">
        <h3 className="font-semibold text-text">{e.nome}</h3>
